...
import com.kotkina.taskManagementSystemApi.security.jwt.JwtUtils;
import com.kotkina.taskManagementSystemApi.services.RefreshTokenService;
...
import com.kotkina.taskManagementSystemApi.web.models.responses.AuthResponse;
import com.kotkina.taskManagementSystemApi.web.models.responses.RefreshTokenResponse;
...
public class SecurityService {
    ...
    public AuthResponse authenticateUser(LoginRequest loginRequest) {
        if (loginRequest == null || !userRepository.existsByEmail(loginRequest.getEmail())) {
            throw new EntityNotFoundException("Пользователь с указанным email не найден");
        }
    ...
    public void register(CreateUserRequest createUserRequest) {
        User user = new User();
        user.setEmail(createUserRequest.getName());
        ...
        return refreshTokenService.findByRefreshToken(requestRefreshToken)
                ...
                .map(userId -> {
                    User tokenOwner = userRepository.findById(userId).orElseThrow(() ->
                            new RefreshTokenException("Токен не найден"));
                    String token = jwtUtils.generateTokenFromHash(tokenOwner.getHash());
